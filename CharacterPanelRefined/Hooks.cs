...
public class Hooks : IDisposable {
    ...
    public event Action<Pointer<AtkUnitBase>>? CharacterStatusOnSetup; 
    public event Action<Pointer<AtkUnitBase>>? CharacterStatusRequestUpdate;
    ...
    private unsafe void* CharacterStatusOnSetupDetour(AtkUnitBase* atkUnitBase, int a2, void* a3) {
        ...
        try {
            CharacterStatusOnSetup?.Invoke(atkUnitBase);
        } catch (Exception e) {
    ...
    private unsafe void* CharacterStatusRequestUpdateDetour(AtkUnitBase* atkUnitBase, void* a2) {
        try {
            CharacterStatusRequestUpdate?.Invoke(atkUnitBase);
        } catch (Exception e) {
