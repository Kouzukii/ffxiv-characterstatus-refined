...
public class Hooks : IDisposable {
    ...
    [Signature("48 89 5C 24 ?? 55 56 57 41 54 41 55 41 56 41 57 48 83 EC 50 48 8B 42 20", DetourName = nameof(GenerateItemTooltipDetour))]
    private readonly Hook<GenerateItemTooltip> generateItemTooltip = null!;
    ...
    public unsafe void* GenerateItemTooltipDetour(AtkUnitBase* addonItemDetail, NumberArrayData* numberArrayData, StringArrayData* stringArrayData) {
        try {
            plugin.UpdateSyncedStats(numberArrayData, stringArrayData);
        } catch (Exception ex) {
            PluginLog.LogError(ex, "Failed to update item tooltip");
        }
        return generateItemTooltip.Original(addonItemDetail, numberArrayData, stringArrayData);
    }

    public void Dispose() {
        characterStatusOnSetup.Dispose();
        characterStatusRequestUpdate.Dispose();
        generateItemTooltip.Dispose();
    }
}
